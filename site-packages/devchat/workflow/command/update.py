...
ZIP_URLS = [
    ...
    # "https://codeload.github.com/kagami-l/new_workflows/zip/refs/heads/main?token=ASD2QBKLDWBTQBTZUEQZMITGBOYUC"
]
# ZIP_URLS = [
...
def _backup(workflow_base: Path, n: int = 5) -> Path:
    """
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for url in candidate_urls:
            try:
                ...
                extracted_dir = tmp_dir_path / f"new_workflows-main" # TODO: use Constant
...
def update_by_zip(workflow_base: Path):
    ...
    if not workflow_base.exists():
        ...
        click.echo(
            f"Updated {workflow_base} by zip. (backup: {backup_zip})"
        )
...
def update_by_git(workflow_base: Path):
    ...
    parent = workflow_base.parent
    tmp_update = parent / f"{WORKFLOWS_BASE_NAME}_new"
    ...
    if not workflow_base.exists():
        ...
        if repo is None:
            ...
            shutil.move(tmp_new, workflow_base)
            click.echo(
                f"Updated {workflow_base} by git. (backup: {backup_zip})"
            )
            return
        ...
        try:
            ...
            click.echo(f"Failed to fetch from origin. Skip update.")
            return
        ...
        try:
            ...
            click.echo(
                f"Updated {workflow_base} from <{local_main_hash[:8]}> to <{remote_main_hash[:8]}>. (backup: {backup_zip})"
            )
        except GitCommandError as e:
