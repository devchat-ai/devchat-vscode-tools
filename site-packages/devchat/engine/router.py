...
def _call_gpt(messages: List[dict],  # messages passed to GPT
    ...
    tools = {} if not use_function_calling else {"tools": _create_tools()}
...
def _run_prompt_command(command_name: str, history_messages, model_name:str):
    """
	run prompt command
	"""
    _, user_chat_dir = init_dir()
...
def run_command(
    ...
    if not command_obj.steps:
        return _run_prompt_command(command, history_messages, model_name)
